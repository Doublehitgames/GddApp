...
import { addonCreated, addonMoved, addonReceipt, addonRow, batchReceipt, deleted, filterSections, json, projectCreated, projectFull, projectIndex, projectReceipt, projectRow, searchProjection, sectionCreated, sectionFull, sectionReceipt, sectionRow, text, touched, } from "./project.js";
function err(e) {
...
export function registerTools(server, client) {
    ...
    server.tool("list_sections", "List a project's sections as an index, sorted by order: id, title, parentId, order, dataId, hasDescription, and the addon TYPES each one carries. Descriptions and addon data are omitted — fetch a specific page with get_section. Narrow the result with subtreeOf / withoutDescription / hasAddonType instead of listing everything and filtering yourself. Pass includeAddons=true for the full dump only when you really need it (on a 185-page project that is over 2 MB).", {
        ...
        try {
            const sections = (await client.listSections(projectId, includeAddons ? undefined : "types"));
    ...
    server.tool("list_addons", "List a section's addons by identity only — id, type, name, group. Their data is omitted; get_section returns the whole page including every addon's values. Pass includeData=true to inline the data anyway (a progression table alone is ~54 KB).", {
        ...
        try {
            const addons = (await client.listAddons(projectId, sectionId));
    ...
    server.tool("update_addon", "Update an addon's name, group, or data. Returns a receipt — {ok, id, type, name, sectionId, updated} — not the addon: a progression table would echo back ~54 KB. Read it back with get_section when you need the saved values.", {
        projectId: z.string(),
        sectionId: z.string(),
        addonId: z.string(),
        ...
        try {
            ...
            const changed = Object.keys((fields.data ?? {}));
    ...
    server.tool("delete_addon", "Remove an addon from a section", {
        ...
        try {
            await client.deleteAddon(projectId, sectionId, addonId);
            return json(deleted("addon", addonId));
        }
        catch (e) {
            return err(e);
        }
    ...
    server.tool("move_addon", "Move an addon from one section to another, keeping its ID. Re-links intra-section refs in the moved addon to the destination's equivalent addons, and when the source section is left without another addon of the same type, rewrites reverse-refs across the project to point at the destination. Singleton addon types already present in the destination cause a 409 unless overwrite=true, which replaces the existing addon in place (keeping its id/group/name). Returns a receipt: { ok, id, type, name, toSectionId, reverseRefsUpdated }.", {
        projectId: z.string(),
        sectionId: z.string().describe("Section UUID where the source addon lives"),
