 ...
 *
 * NOTE: twin of lib/mcp/instructions.ts (the remote /api/mcp server).
...
A description is prose with design value — context, role, flavour, how the thing connects to the rest of the game. It is not a spec sheet. Balance numbers — prices, times, quantities, levels, rates — go stale every time the game is retuned, and a page built on them rots with it; describe the relation instead ("the shortest production cycle on the ranch") and leave the values out unless you were asked for them. Give the page a shape instead of one long column of prose: a heading breaks a page that runs past a screen, concrete parallel data belongs in a table rather than diluted across sentences, a warning or a design decision reads as a callout, and an image sits beside the text that explains it in columns. Send \`content\` as markdown and the server derives \`contentBlocks\` from it, so the two cannot disagree — markdown alone already gives you headings, tables, callouts, lists, quotes, code and images, so it is the normal path. Hand-build \`contentBlocks\` only for what markdown cannot say: embeds, columns, per-block colour or alignment. Always pair blocks with a plain-text \`content\` for search.
