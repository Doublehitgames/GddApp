...
- **Rich descriptions** — send the description as markdown and the server derives the formatted blocks from it: headings, tables, callouts, lists, quotes, code and images all come from markdown alone. Hand-build `contentBlocks` for what markdown cannot say — an embed, an image side by side with the text that explains it, a coloured block; `get_content_blocks_guide` explains the format on demand.
