...
| Tipos de addon | progressionTable, xpBalance, currency, globalVariable, economyLink, inventory, production, **craftTable** (Mesa de Produção — agrega receitas Production com unlocks e drag-order, novo em 2026-04-22), dataSchema, attributeDefinitions, attributeProfile, attributeModifiers, fieldLibrary, exportSchema, **richDoc** (BlockNote/Notion-like, novo em 2026-04-20) |
| CraftTable (Mesa de Produção) | `components/CraftTableAddonPanel.tsx`, `components/CraftTableAddonReadOnly.tsx`. Cada entry referencia uma seção que contém um Production addon; tem `category`, `order`, `unlock` (level/currency/item com `enabled` por slot) e `hidden`. Singleton por addon group (um por página). Drag-and-drop via dnd-kit. Production Panel/ReadOnly em modo recipe mostram "Produzida na mesa X" quando há Craft Table referenciando. |
| Inventory.category via Field Library | `InventoryAddonDraft.categoryLibraryRef?` (libraryAddonId + entryId). Quando setado, a categoria vem do entry da Biblioteca (mesmo padrão de Data Schema e Progression Table). UI usa `LibraryLabelPath` pra renderizar labels hierárquicos tipo "Consumível/Comida/Selvagem" como chips. |
| ExportSchema craftTable support | Array sources novos: `craftTable`, `productionIngredients`, `productionOutputs`. Bindings novos: `entryField` (craft entry), `productionField` (segue entry.productionRef), `itemField` (ingrediente/output). Refs (productionRef, xpAddonRef, currencyAddonRef, itemRef, outputRef) resolvem pro `dataId` da seção-alvo, não UUID. `resolveExportSchemaWithPreview` devolve `{result, nodeValueMap}` pro editor mostrar preview inline. |
...
- **Coluna `balance_addons` ausente**: a API também tem fallback para sync continuar sem addons quando a coluna ainda não existe; aplicar `lib/supabase/add_sections_balance_addons.sql` para persistência completa.
