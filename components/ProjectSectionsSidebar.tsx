...
export default function ProjectSectionsSidebar({ projectId, projectSlug }: Props) {
  ...
  const createSection = (title: string, parentSectionId: string | null): string | undefined => {
    try {
      if (parentSectionId) {
        return addSubsection(projectId, parentSectionId, title, "", sectionAuditBy);
      }
      return addSection(projectId, title, undefined, sectionAuditBy);
    } catch (e) {
      if (
        e instanceof Error &&
  ...
  const handleAddByContext = () => {
    ...
    const created = createSection(title, resolvedSectionId ?? null);
...
function SectionTree({
  ...
  const matchesFilters = (section: any): boolean =>
    matchesSearch(section) && matchesTags(section);
  ...
  const totalMatches =
    searchTerm.trim() || selectedTagFilters.length > 0
