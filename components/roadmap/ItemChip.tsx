...
interface Props {
  ...
  /** When true, hides all edit controls (member view) */
  readOnly?: boolean;
}
...
export default function ItemChip({ item, onUpdate, onDelete, dragHandleListeners, dragHandleAttributes, readOnly }: Props) {
  const { t } = useI18n();
        ...
        <div className="w-full max-w-xl rounded-xl border border-gray-700 bg-gray-900 shadow-2xl p-3 flex flex-col gap-2.5 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex gap-3">
            ...
            {readOnly ? (
              ...
              item.thumbUrl ? (
                <div className="relative shrink-0 w-28 h-28 rounded-lg border border-gray-700 bg-gray-800/50 overflow-hidden">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-2">
              {readOnly ? (
                /* Read-only: static title */
                <p className="w-full px-2.5 py-1.5 text-sm text-white leading-snug">{item.title}</p>
              ) : (
                <CommitTextInput
                  value={item.title}
              ...
              {/* Status — buttons when editing, static badge when read-only */}
              {readOnly ? (
                <div className="flex items-center gap-1.5">
                  <span className={`flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium ${STATUS_STYLES[item.status].chip} ${STATUS_STYLES[item.status].text.split(" ")[0]}`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${STATUS_STYLES[item.status].dot}`} />
                    {t("roadmap.status." + item.status)}
                  </span>
                  {item.tag && ITEM_TAG_CONFIG[item.tag] && (
                    <span className={`rounded border px-1.5 py-0.5 text-[10px] font-bold ${ITEM_TAG_CONFIG[item.tag].style}`}>
          ...
          {/* Footer: public toggle + delete — hidden for read-only members */}
          {!readOnly && (
