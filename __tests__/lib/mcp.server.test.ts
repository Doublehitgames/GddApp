...
describe("write tools return receipts, not records", () => {
  it("update_section answers with {ok, id, title, updated, updatedAt}", async () => {
    ...
    // The page's own text never reaches the agent.
...
describe("list_sections filters", () => {
  const tree = [
    { id: "root", title: "Animais", order: 0, content: "x" },
    { id: "kid1", parentId: "root", title: "Galinha", order: 1, content: "tem texto" },
    { id: "kid2", parentId: "root", title: "Vaca", order: 2, content: "", contentBlocks: [] },
    { id: "grand", parentId: "kid2", title: "Leite", order: 3, content: "", contentBlocks: [] },
    { id: "other", title: "Economia", order: 4, content: "y" },
  ];
...
describe("listings return index rows", () => {
  it("list_sections omits the descriptions themselves", async () => {
  ...
  it("get_project returns the index, not the territory", async () => {
    ...
    expect(JSON.stringify(index)).not.toContain("bota ovos");
  });
});
...
describe("tool descriptions teach the new defaults", () => {
  it("the write tools say they return a receipt", () => {
    const h = harness();
    for (const tool of ["update_section", "create_section"]) {
  ...
  it("every write exposes the returning escape hatch", () => {
    const h = harness();
    for (const tool of ["update_section", "create_section", "update_project", "create_project"]) {
  ...
  it("the read tools do not, because they already return everything", () => {
    const h = harness();
    for (const tool of ["get_section", "get_project", "list_sections"]) {
