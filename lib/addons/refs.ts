...
function relinkBindingToSection(
  ...
  switch (binding.source) {
    ...
    case "inventory": {
      const id = targetAddonIdOfType(targetAddons, "inventory");
      return id ? { ...binding, addonId: id } : undefined;
    }
  ...
  if (type === "dataSchema" || type === "genericStats") {
    ...
    if (Array.isArray(entries)) {
      for (const entry of entries) {
        ...
        if (
          (binding?.source === "production" || binding?.source === "crop" || binding?.source === "inventory") &&
  ...
  if (addon.type === "dataSchema" || addon.type === "genericStats") {
    ...
    if (Array.isArray(entries)) {
      for (const e of entries) {
        if (e.binding?.source === "production" || e.binding?.source === "crop" || e.binding?.source === "inventory") ids.add(e.binding.addonId);
      }
    }
